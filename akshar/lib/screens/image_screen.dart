...
class _ImageScreenState extends State<ImageScreen> {
  ...
  Future<void> _pickImage(BuildContext context) async {
    ...
    state.setProcessing();
    try {
      final results = await RecognitionService.instance.recognizeFromImage(
        imagePath: path, isDigitMode: state.isDigitMode,
      );
      if (results.isEmpty) {
        state.setError('Nothing found in image.');
      } else {
  ...
  void _reset(RecognitionState state) {
          ...
          child: Column(
            children: [
                              ...
                              if (state.status == RecognitionStatus.processing)
                                Container(
                                  ...
                                  child: const Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
                                    ...
                                    SizedBox(height: 12),
                                    Text('Scanning…', style: TextStyle(fontFamily: 'Tiro', fontSize: 13, color: AppTheme.teal, letterSpacing: 1)),
                                  ])),
                                ),
                            ])
              ...
              Row(
                children: [
                  if (_imagePath != null) ...[
                    _OutlineBtn(icon: Icons.delete_outline_rounded, label: 'Remove', onTap: () => _reset(state)),
                    const SizedBox(width: 12),
                  ],
                  Expanded(child: _GoldBtn(
                    icon: Icons.photo_library_rounded,
                    label: _imagePath == null ? 'Choose Image' : 'Change',
...
class _ImagePlaceholder extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Center(
    child: Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      const SizedBox(height: 14),
      const Text('Choose Image', style: TextStyle(fontFamily: 'Tiro', color: AppTheme.textSub, fontSize: 15)),
      const SizedBox(height: 4),
      Text('Tap to select from gallery', style: TextStyle(fontSize: 11, color: AppTheme.textSub.withValues(alpha: 0.5))),
    ]),
  );
}
...
class _GoldBtn extends StatelessWidget {
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: Container(
      ...
      decoration: BoxDecoration(
        color: AppTheme.gold, // flat matte — was goldGradient
        borderRadius: BorderRadius.circular(14),
      ),
